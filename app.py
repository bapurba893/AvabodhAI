...
with st.sidebar:
    ...
    # tenant_id + org_unit_id. This UI has no gateway in front of it, so
    # there's no headers to read them from; the operator types both in
    # directly. Every document processed in this session is attributed
    # to this tenant + department.
    ...
    org_unit_id = st.text_input(
        "Org Unit ID",
        value=st.session_state.get("org_unit_id", ""),
        help="Department within the tenant. Documents are isolated per department, not just per tenant.",
    )
    st.session_state["org_unit_id"] = org_unit_id

    missing_ids = []
    if not tenant_id.strip():
        missing_ids.append("Tenant ID")
    if not org_unit_id.strip():
        missing_ids.append("Org Unit ID")
    if missing_ids:
        st.warning(f"Enter {' and '.join(missing_ids)} above to process or view documents.")

    is_ground_truth = st.checkbox(
        "Mark as ground truth",
        value=False,
        help="Only ground-truth documents are ever retrievable by chat/search. Unchecked by default.",
    )
...
with col_upload:
    ...
    process_btn = st.button(
        "Generate Summary",
        disabled=uploaded_file is None or not tenant_id.strip() or not org_unit_id.strip(),
        use_container_width=True,
    )
    if uploaded_file is not None and (not tenant_id.strip() or not org_unit_id.strip()):
        st.caption("⚠️ Enter both Tenant ID and Org Unit ID in the sidebar before processing.")
